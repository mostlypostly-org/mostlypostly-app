...
router.post("/signup", (req, res, next) => salonLogoUpload(req, res, next), async (req, res) => {
  try {
    ...
    if (promoCode && phone) {
      sendViaTwilio(phone, `Welcome to MostlyPostly! Your 30-day free trial starts when you select a plan. As a founding member, use promo code ${promoCode} at checkout to lock in your founder rate. Questions? Reply here anytime.`).catch(smsErr => {
