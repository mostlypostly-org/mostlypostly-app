...
router.get("/", requireAuth, (req, res) => {
  ...
  if (vendors.length === 0) {
    ...
    vendorCards = vendors.map(([vendorName, items]) => {
      ...
      const nonExpired = items.filter(c => !c.expires_at || c.expires_at >= new Date().toISOString().slice(0, 10));
      const expired    = items.filter(c => c.expires_at && c.expires_at < new Date().toISOString().slice(0, 10));
          ...
          <div class="px-5 py-4 flex items-center justify-between gap-4">
            <div>
              ...
              <p class="text-xs text-mpMuted">
                ${nonExpired.length} active campaign${nonExpired.length !== 1 ? "s" : ""}
            ...
            <form method="POST" action="/manager/vendors/toggle?salon=${salon_id}" class="flex items-center gap-3">
              ...
              <button type="submit" ${!isPro ? "disabled" : ""}
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none
                       ${enabled ? "bg-mpAccent" : "bg-mpBorder"} ${!isPro ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}">
...
router.post("/toggle", requireAuth, (req, res) => {
  ...
  if (!isPro) return res.redirect(`/manager/vendors?salon=${encodeURIComponent(salon_id)}`);

  const { vendor_name, enabled } = req.body;
  if (!vendor_name) return res.redirect(`/manager/vendors?salon=${encodeURIComponent(salon_id)}`);
  ...
  res.redirect(`/manager/vendors?salon=${encodeURIComponent(salon_id)}`);
});
